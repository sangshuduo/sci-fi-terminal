...
fn panel_rows<'a, M: Clone + 'a>(
    ...
    vec![
        ...
        toggle(
            "Peer globe",
            settings.draft.panels.network.globe,
            move |v| wrap(SettingsMsg::Globe(v)),
        ),
        toggle(
            "Rotate globe (off while reduced motion is on)",
            settings.draft.panels.network.globe_rotate,
            move |v| wrap(SettingsMsg::GlobeRotate(v)),
        ),
        (
            "GeoIP database (.mmdb)",
            text_input(
