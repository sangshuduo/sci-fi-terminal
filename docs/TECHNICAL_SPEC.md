...
- **Process and network monitor.** A process panel shows the top processes by CPU usage, with name, PID, CPU and memory only. It never collects command lines or environment variables. A network panel shows per-interface transfer rates and up to 200 active sockets. It samples only while visible, at least every 1 s for processes and every 2 s for the network. GeoIP is **offline and opt-in**: the user points `panels.network.geoip_database` at a local MaxMind-format `.mmdb` file. The app never downloads a database and never sends peer addresses anywhere; the only outbound request is the separate opt-in home lookup (ADR-007). Only public addresses are looked up.
...
- **Panels.** System (CPU, memory, swap, top processes), Network (globe, interface rates, connections) and Directory are separate, independently toggled panels. Directory docks left; System and Network dock right as separate cards.
- **Home spot (ADR-007).** An opt-in public-IP lookup (off by default) finds this machine's city with the offline GeoIP database and marks it on the globe with a flashing spot (steady under reduced motion).
- **Peer globe (ADR-006).** The network panel draws an original wireframe globe of public-domain Natural Earth coastlines, with markers for GeoIP-located peers. It rotates at ≤ 30 Hz only while visible and focused, and only when reduced motion is off. Otherwise it is static.
